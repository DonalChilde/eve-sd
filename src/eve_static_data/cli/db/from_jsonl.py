...
def from_jsonl(
    ...
    db_name: Annotated[
        ...
        typer.Option(
            ...
            "which will create a file named `eve_static_data_{build_number}_jsonl.db`",
            show_default=True,
        ),
    ] = None,
    ...
    """Import SDE data from JSONL files into the database."""
    sde_metadata = load_sde_metadata(sde_jsonl_path)
    typer.echo(
        f"Starting import of SDE data from {sde_metadata.source_format} files into "
        f"database at {db_directory} for SDE build {sde_metadata.buildNumber}..."
    )
    if db_name is None:
        db_name = f"eve_static_data_{sde_metadata.buildNumber}_jsonl.db"
    ...
    import_jsonl_sde_to_db(
        sde_jsonl_path, connection=connection, serialization_format=serialization_format
    )
    end_time_ns = perf_counter_ns()
    elapsed_time_s = (end_time_ns - start_time_ns) / 1_000_000_000
    typer.echo(
        f"Finished importing SDE data from {sde_metadata.source_format} files into database at {db_path} in {elapsed_time_s:.3f} seconds."
    )
    logger.info(
